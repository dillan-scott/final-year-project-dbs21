...
% \todo{Citations for limitations and weaknesses section}
...
\subsection{Robust Generative Modelling (RRBM-DD)}
\todo{Maybe remove if no longer relevant}
Korycki and Krawczyk~\cite{Korycki_2022} introduce the Robust Restricted Boltzmann Machine for Drift Detection (RRBM-DD), a generative neural network that detects drift by monitoring the reconstruction error. Unlike classical models, the RRBM-DD incorporates specific mechanisms in an attempt to filter out malicious data during updates.

To counter instance-based poisoning, it uses a robust gradient descent method with M-estimators that softly truncate the influence of extreme loss values, preventing outliers from skewing the weight updates. For concept-based poisoning, the model employs a robust energy function with a gating mechanism and a Gaussian noise model. This allows the network to dynamically turn off neurons affected by adversarial clusters, ensuring the detector remains sensitive to genuine drift while ignoring malicious artefacts.

\subsection{Contrastive Learning and Hybrid Distance Metrics}
\label{sec:contrastive_ncm}
\todo{Rewrite section to be way more in-depth}
...
Drift detection is then performed on this latent space using a Nearest Class Mean (NCM) classifier equipped with a hybrid distance metric. This metric combines both Euclidean distance (instance-level fidelity) and Riemannian divergence (group-level geometric fidelity). The approach relies on the fact that adversarial subspaces typically lie on low-probability manifolds distinct from the true data distribution. Additionally, the system supports a recursive update mechanism which automatically discovers and defines new classes from accumulated drift vectors.