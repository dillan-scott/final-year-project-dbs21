...
\begin{enumerate}
    ...
    \item \textit{Reoccurring concepts} refers to when previously seen concepts reappear after some time, such as in seasonal or cyclical patterns~\cite{Gama_2014}. They may reappear both suddenly or incrementally~\cite{Guzy_2020}.
\end{enumerate}
...
\missingfigure{Maybe a wireframe of drift detection architecture}
...
Importantly, these limitations can be mapped onto the general architecture of drift detectors introduced earlier in~\cref{sec:drift detection architecture}. Choices made at the level of the \textit{observation source} (e.g. error rates or feature statistics), the \textit{summary mechanism} (e.g. windowing or cumulative statistics), and the \textit{change test} (e.g. hypothesis tests or thresholds) impose constraints on what types of drift can be reliably identified and how quickly.
...
Consequently, distribution-based detectors may generate false positives, triggering model retraining or data forgetting even when performance remains stable (there is no \textit{real} drift). Such unnecessary adaptation can be computationally expensive and may temporarily degrade performance.
...
In such settings, the drift process is no longer passive. Instead, the data stream itself becomes an attack surface, and the drift detector becomes a target. Adversarial drift extends traditional data poisoning attacks by exploiting temporal dynamics and specific detector behaviours. Rather than corrupting a static training set, the adversary seeks to control \textit{when} adaptation is triggered, \textit{what data} is included during retraining, and \textit{which information} is forgotten. This introduces a new class of threats that cannot be fully explained by classical adversarial learning models alone~\cite{Kloft_2010}.
...
A primary objective could be \textit{performance degradation}, where the adversary seeks to reduce the predictive accuracy or increase the loss of a system~\cite{Sethi_2018}. Unlike poisoning attacks, adversarial drift may be gradual, ensuring that degradation accumulates slowly while remaining below detection thresholds.

This could be seen as a potential secondary objective: \textit{evasion of detection}. Here, the adversary aims to avoid the activation of the drift detector while pursuing their primary objective. This could involve crafting inputs that move the decision boundary while preserving the statistics monitored, hence exploiting the blind spots of classical detectors.

Alternatively, an adversary may attempt \textit{forced adaptation}, deliberately triggering false drift alarms to cause unnecessary retraining or forgetting. Such behaviour can destabilise the learning process, increase computational costs, or induce catastrophic forgetting of valid historical concepts~\cite{Korycki_2022}. Conversely, an adversary could seek to \textit{block adaptation}, injecting contradictory data points during a period of valid concept drift, in an attempt to nullify the adaptation process~\cite{Korycki_2022}.

Finally, adversaries may seek \textit{control over the learned concept}, gradually forcing the system to learn a decision boundary favourable to the attacker’s goals. In this case, the detector is actively exploited as a mechanism for injecting the poisoned instances during adaptation cycles.
...
\end{equation}
where $n_c$ is the number of training samples observed for class $c$. The assigned class for an incoming sample with embedding $\mathbf{h}_j$ is the one minimising a hybrid distance,
\begin{align}
...
The two terms in the metric are intended to capture both instance and group level fidelity. The Euclidean term enforces instance level fidelity, while the Riemannian term enforces group level fidelity. Their combination is claimed to be robust against adversarially-crafted inputs by exploiting three properties of adversarial subspaces~\cite{Kuppa_2022}: (a) they are found in lower-probability regions compared to the data manifold; (b) their class distributions tend to vary from their closest natural sub-manifold; and (c) adversarial samples closest neighbour is their unperturbed source, rather than any other point in the training or test set.

The same hybrid distance is also used to identify drifted samples. If a sample's minimum hybrid distance to every known prototype is large, it is taken as evidence that it does not belong to any previously observed class. In practice this is operationalised through a threshold (say $T_{drift}$), although Kuppa and Le-Khac~\cite{Kuppa_2022} do not formally specify how it should be derived. The calibration procedure adopted in this thesis is detailed in~\cref{ch:target_system_threat_model}.
...
\end{align}
When the accumulated displacement crosses a pre-defined threshold $T$,
\begin{equation} \label{eq:concept-threshold}
    \|\Delta_{drift}^{t \to t+1}\| > T,
\end{equation}
new mean class prototypes $h_{new,i}$ are registered, both the accumulator and buffer are reset, and the autoencoder is retrained on new data that includes the newly identified class. Following the rehearsal principles in \cref{sec:exemplar memory}, all of the registered classes are represented at retraining time by fixed-size exemplar sets selected via herding (\cref{eq:herding}), so that storage remains bounded and the training set remains faithful to the previously seen data.
