\chapter{Evaluation}
\label{ch:evaluation}
\todo[color=purple]{Proof read and polish the text of this chapter.}
...
\input{./evaluation/baseline_adaptation_table.tex}

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=\linewidth]{../experiments/contrastive_ncm/figures/rq2_streaming_timeseries.pdf}
%     \caption{Per-batch stream $F_1$ and the \texttt{DoS} per-class false-negative rate over the week, baseline (no attack) versus a $5\%$ benign-labelled injection. Aggregate $F_1$ is barely moved while the per-class FNR is driven from near zero to $0.642$ --- the damage the headline metric conceals.}
%     \label{fig:eval-rq2-ts}
% \end{figure}
\clearpage
...
\input{../experiments/contrastive_ncm/tables/attack_fri_summary.tex}

So, the boundary cannot be moved against the genuine class, but it can be circumvented. The channel admits a backdoor, which succeeds precisely because it does not contend with the clean decision boundary. Rather than just mislabelling \texttt{DoS} as benign, the adversary also stamps them with a small feature-space trigger, teaching $\mathcal{M}$ that ``\texttt{DoS}-features $+$ trigger $\Rightarrow$ benign''. The trigger is inert on an un-poisoned learner -- it moves the \texttt{DoS} false-negative rate from $0.012$ to $0.014$ -- but after the backdoor is planted a \emph{triggered} \texttt{DoS} flow evades $\mathcal{M}$ with false-negative rate $0.69$, while \emph{clean} \texttt{DoS} is still caught (with false-negative rate $0.03$). It is also worth noting, the backdoor is durable: the adversary injects only around the genuine mid-week fire and then stops, yet the backdoor persists reagrdless of any further retrains. This is because of all the harvested concept exemplars that carry the trigger being replayed into every subsequent rebuild -- the never-evict exemplar memory (\cref{sec:impl-retrain}) turned against the system. Here, the robust learner resists having its boundary moved, but not having a hidden shortcut installed beside it.

\input{../experiments/contrastive_ncm/tables/rq2_backdoor.tex}
...
\input{../experiments/contrastive_ncm/tables/rq3_pgd_meanshift.tex}
\todo[color=purple]{Position the white-box meanshift attack table properly.}
...
\input{../experiments/contrastive_ncm/tables/rq3_pgd_meanshift_greybox.tex}
\todo[color=purple]{Position the grey-box meanshift attack table properly.}
...
\input{../experiments/contrastive_ncm/tables/rq3_gate_forcing.tex}
\todo[color=purple]{Position the gate ablation (forcing) table properly.}

\input{../experiments/contrastive_ncm/tables/rq3_gate_suppression.tex}
\todo[color=purple]{Position the gate ablation (suppression) table properly.}
