...
\todo[color=blue]{Describe the implementation of each attack.}
We close with the implementation of the attacks selected for evaluation in \cref{ch:attack_taxonomy_objectives}. The results are reported in \cref{ch:evaluation}.

% \subsection{The Injection Model}
% \label{sec:impl-injection}
% The adversary acts by injecting crafted flows into the stream. We model this as a per-batch \textit{injection} rather than replacement: at injection fraction $p$ the attacker adds $\lfloor\, p\,n_t \,\rfloor$ samples to a batch of $n_t$ genuine samples, so the crafted flows make up a fraction $p/(1+p)$ of the batch (\cref{def:injection_budget}). The injection rate may vary over the stream through a schedule function -- supporting sustained, oscillating, and ``poison-then-clean'' attack patterns -- and the attacker can choose the label under which the injected flows are presented.

% \subsection{Composition and Label-Flipping Attacks}
% \label{sec:impl-labelflip}
% The simplest attacks manipulate what enters the concept-discovery buffer and under what label. Injecting novel-class flows under a \textit{benign} label holds the buffer's makeup below the firing gate, suppressing firing even as genuine drift accumulates; injecting them under their \textit{true} label instead drives the attack-labelled fraction past the gate to force an early, attacker-timed retraining event. The same two manipulations thus exercise the trigger channel's firing gate and, when a rebuild does occur, the composition channel's retraining set.

% \subsection{Feature-Space (PGD) Attacks}
% \label{sec:impl-pgd}
% The remaining attacks perturb features directly, against the detector's differentiable drift score --- the minimum hybrid NCM distance of an embedding to the known prototypes, which is differentiable in the input. All three are projected gradient descent under an $L_\infty$ budget $\epsilon$ (swept in \cref{ch:evaluation}): $50$ steps at step size $\alpha = 2.5\,\epsilon/50$, projecting onto the $\epsilon$-ball each step and then clipping to the per-feature observed range so that the crafted flows stay within plausible feature values.

% The three variants target different statistics. The first maximises each sample's individual drift score, pushing known-class flows past $T_{drift}$ so that they are flagged as drifted; this saturates per-sample detection but scatters the embeddings, leaving their mean almost unmoved. The second pushes every embedding coherently along a single fixed latent direction --- a deliberately naive attempt at the displacement the concept-discovery accumulator integrates, retained as a foil. The third is the aimed attack: it optimises two pools of flows so that their encoded batch means are driven maximally apart ($\max \lVert \mu_A - \mu_B \rVert$ in latent space), which is the lever that forces a firing event.

% Each attack is run white-box, perturbing against the victim detector itself, and grey-box, perturbing against a surrogate detector and transferring the crafted flows to the victim. The surrogate shares the victim's architecture, hyperparameters and withheld classes and is drawn from the same training pool, differing only by an independent random seed --- a different train/calibration partition and weight initialisation. It thus models an adversary who knows the system design and the data distribution and trains their own detector; because its training data overlaps the victim's, the measured transfer is likely more favourable than a fully data-disjoint surrogate would achieve, which we leave to future work.
