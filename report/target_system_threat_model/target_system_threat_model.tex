...
The system couples $\mathcal{D}$ and $\mathcal{M}$ through a trigger-dependent adaptation mechanism: in general $\mathcal{M}$ is static, only being updated when $\mathcal{D}$ registers a new concept. On each incoming batch, $\mathcal{D}$ flags drifted samples and accumulates them in the concept-discovery buffer; once the accumulated displacement crosses $T$, a new prototype is registered, the encoder is retrained, and $\mathcal{M}$ is replaced by a new model trained on the updated dataset. The specific composition of the retraining dataset at each trigger is an architectural choice we return to in \cref{ch:implementation}. This coupling is intended to allow $\mathcal{M}$ to inherit both $\mathcal{D}$'s robustness properties and its decision latency. 

\begin{figure}[h]
    \centering
    \includegraphics[width=\linewidth]{target_system_threat_model/target_architecture.drawio.pdf}
    \caption{The architecture of the target system. $\mathcal{D}$ and $\mathcal{M}$ both consume the input stream in parallel: $\mathcal{M}$ produces classifications while $\mathcal{D}$ monitors for drift. When $\|\Delta\| > T$, the coupling layer triggers a retraining event (dashed) that updates $\mathcal{D}$ (registering a new prototype and retraining the encoder) and replaces $\mathcal{M}$ with a new model trained on the updated dataset.}
